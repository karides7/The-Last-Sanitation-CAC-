...
let numbersOnly = gumballArray[gumballColor].replace();
console.log(gumballArray[gumballColor].replace());
...
gumballImage.addEventListener(`click`, function () {
  ...
  if (gumballImage.src.endsWith("gumball.png")) {
    ...
    localStorage.setItem(`microplastics`, currentMicroplastics + numbersOnly);
    console.log(reward, calcReward(), calcReward(reward));
    prize.textContent = `You Win ${rewardArray[calcReward(reward)]}`;
  }
});
